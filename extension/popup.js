...
captureBtn.addEventListener('click', async () => {
  ...
  // Fetch the active task's logId so the background doesn't have to rely on
  // in-memory currentTaskLogId (which resets on service worker restart)
  const activeTaskData = await loadActiveTaskName();
  const captureLogId = activeTaskData ? (activeTaskData.logId || activeTaskData.log_id || null) : null;

  chrome.runtime.sendMessage({ type: 'MINUTEFLOW_MANUAL_CAPTURE', logId: captureLogId }, (response) => {
    setTimeout(async () => {
      captureBtn.disabled = false;
