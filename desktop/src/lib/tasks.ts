...
/** On-queue and in-progress tasks assigned to this VA, newest first — the
 *  same set the web dashboard's Assigned Tasks widget shows. */
export async function fetchAssignedTasks(userId: string): Promise<VAAssignedTask[]> {
  ...
  const rows = await query<VAAssignedTask[]>("assigned_task_assignees", {
    filters: `va_id=eq.${userId}&status=in.(${statusList})&select=${VA_SELECT}&order=assigned_at.desc`,
  });
