...
export async function GET(request: Request) {
  ...
  const assigneeSelect =
    "id, va_id, status, log_id, notes, accuracy_score, assigned_at, updated_at, sort_order, instructions, instructions_locked";
  const taskSelect =
    `id, account, project, project_id, parent_task_id, pay_type, category, task_name, task_detail, task_notes, link, due_date, due_time, start_date, end_date, start_time, end_time, planned_minutes, archived_at, deleted_at, created_by, created_at, updated_at, status, paid_manually, assigned_by, instructions, instructions_locked, review_required, review_required_locked, revision_count, fixed_pay_task_id, recurring_template_id, spawned_template_id, fixed_pay_tasks(rate), assigned_by_profile:profiles(id, full_name, username), projects(id, name), task_todos(id, text, sort_order),
  ...
  const vaSelectString = `id, va_id, status, log_id, notes, accuracy_score, assigned_at, updated_at, sort_order,
     assigned_tasks(id, account, project, project_id, parent_task_id, category, recurring_template_id, spawned_template_id, task_name, task_detail, task_notes, link, due_date, due_time, start_date, end_date, start_time, end_time, planned_minutes, archived_at, deleted_at, created_by, created_at, updated_at, status, paid_manually, assigned_by, instructions, instructions_locked, review_required, review_required_locked, revision_count, fixed_pay_task_id, fixed_pay_tasks(rate), projects(id, name), task_todos(id, text, sort_order))`;
...
export async function POST(request: Request) {
  ...
  const {
    ...
    /** Reverse link: this task SPAWNED this recurring template (from the
     *  "also save as a recurring template" checkbox). Distinct from
     *  recurring_template_id above, which means the opposite direction —
     *  this task was GENERATED BY that template. */
    ...
    /** Minutes the task takes when it has no specific hours. Null when it has a
     *  block, whose own length is already its duration. */
  ...
  const { data: task, error: taskError } = await adminSupabase
    ...
    .insert({
      ...
      spawned_template_id: spawned_template_id ?? null,
