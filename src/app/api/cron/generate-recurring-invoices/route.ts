 ...
 * `is_recurring = true` (a series template carrying `recurring_series_id`), it
 * generates next month's invoice for the just-completed month as a DRAFT that
 * needs review — it is never sent to the client here.
 *
 * - Hourly / timelog → same client + rate + settings, RECOMPUTES from the
 *   just-completed month's billable `time_logs`. Always generated.
 * - Fixed / custom → clones last month's `custom_line_items` + totals verbatim,
 *   but ONLY if the previous invoice in the series is already `paid` (Toni's
 *   rule: don't bill the next month until the last one is settled).
 *
 * Each draft gets a random `send_code`; the review page requires typing it to
 * unlock "Send". Toni is notified with a review link (email now; the notify
 * layer is channel-agnostic for future Telegram/SMS).
 *
 * Secured by CRON_SECRET. Mirrors ../recurring-tasks + ../invoice-reminders.
 */
...
function getTimezoneParts(date: Date, timeZone: string) {
  ...
  return {
    ...
    month: Number(mapped.month || "1"), // 1–12
...
/** 6-char human-typable code (no ambiguous chars) for the send gate. */
...
async function handleCron(request: NextRequest) {
  ...
  if (tmplError) {
    return Response.json({ error: tmplError.message }, { status: 500 });
  }
  ...
  for (const template of (templates ?? []) as InvoiceRow[]) {
    ...
    // Already generated for this period? (dedup)
    ...
    // Latest invoice in the series → clone source + "previous paid?" check
    ...
    // Build totals / line items per type
    let subtotal = 0;
    let totalHours = 0;
    let timelogItems: {
    ...
    if (isHourly) {
      ...
      // Exclude logs already committed to active invoices
    ...
    // Insert draft (retry on invoice_number collision)
    ...
    // Notify Toni with a review link + the send code (email now)
    if (notifyEmail) {
  ...
  return Response.json({ generated, period: { periodStart, periodEnd }, results });
}
