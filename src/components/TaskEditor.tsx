...
const TaskEditor = forwardRef<TaskEditorHandle, TaskEditorProps>(function TaskEditor({
  ...
  // This task is an OCCURRENCE of a recurring template (assigned_tasks.
  // recurring_template_id) — a different relationship than existingTemplateId
  // above (assigned_tasks.spawned_template_id, a one-off that also spawned its
  // own companion template). A task can't be both, but the "Also save as a
  // recurring template" checkbox below only ever checked for the spawned case,
  // so an occurrence loaded with the checkbox unticked and available — as if
  // it were a plain one-off, when it's already generated by a series. Fetched
  // so the read-only note below can say what it repeats, not just that it does.
  const linkedSeriesTemplateId = (initialTask?.recurring_template_id as string) ?? null;
  const [linkedSeriesTemplate, setLinkedSeriesTemplate] = useState<{ recurrence_type?: RecurrenceType } | null>(null);
  useEffect(() => {
    if (!linkedSeriesTemplateId) return;
    fetch(`/api/recurring-task-templates?id=${linkedSeriesTemplateId}`, { cache: "no-store" })
      .then((r) => r.json())
      .then((d) => setLinkedSeriesTemplate(d.template ?? null))
      .catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    <div className="space-y-3">
      ...
      <Section title="Schedule" {...accordionProps("schedule")} warning={missingSchedule}>
        ...
        {!templateMode && belongsToSeries && (
          /* This task IS an occurrence of a template (recurring_template_id) —
             offering to spin up ANOTHER, separate template from it here would
             be a second, conflicting source of truth. Point at the real one
             instead of pretending this task is still a one-off. */
          <div className="rounded-lg border-2 border-amber bg-amber-soft/40 p-3 space-y-1">
            <p className="flex items-center gap-1.5 text-[12px] font-bold uppercase tracking-wide text-espresso">
              <RecurringBadge fromTemplateId={linkedSeriesTemplateId} />
              Part of a recurring template
            </p>
            <p className="text-[11px] text-stone">
              {linkedSeriesTemplate?.recurrence_type
                ? `Repeats ${linkedSeriesTemplate.recurrence_type}. `
                : ""}
              This task was generated from that template — change its schedule there, not on this one occurrence.
            </p>
            <a
              href="/productivity/assignment?tab=recurring"
              className="inline-block text-[11px] font-semibold text-terracotta hover:underline"
            >
