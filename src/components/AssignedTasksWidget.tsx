...
export default function AssignedTasksWidget({
  ...
  const assignedTasks = tasks.filter((task) => task.status === "on_queue" || task.status === "in_progress");

  const renderTaskSection = (title: string, sectionTasks: VAAssignedTask[]) => (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between px-0.5 pb-1">
        <h4 className="text-[11px] font-semibold uppercase tracking-wide text-walnut">{title}</h4>
        <span className="text-[10px] font-semibold text-stone">{sectionTasks.length}</span>
      </div>
        ...
        return (
          <Fragment key={task.id}>
            <div className="flex flex-col gap-1.5 py-2.5 px-3 rounded-lg border border-sand bg-white hover:bg-cream transition-colors">
              ...
              {isExpanded && (
                <>
                  {accountProject && <div className="text-[11px] text-bark pl-[18px]">{accountProject}</div>}
        ...
        <div className="p-[18px_20px]">
          {loading ? (
            ...
            <div className="space-y-4">
              {availableTasks.length > 0 && renderTaskSection("Available Tasks", availableTasks)}
              {assignedTasks.length > 0 && renderTaskSection("Assigned Tasks", assignedTasks)}
            </div>
          )}
        </div>
      )}
